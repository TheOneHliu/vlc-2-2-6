...
static int OpenWithCookies( vlc_object_t *p_this, const char *psz_access,
                            unsigned i_redirect, vlc_array_t *cookies )
{
	msg_Info_Br("hliu:%s","call OpenWithCookies");  
    access_t     *p_access = (access_t*)p_this;
    access_sys_t *p_sys;
    char         *psz, *p;
...
{
	msg_Info_Br("hliu:%s","call Close");
    access_t     *p_access = (access_t*)p_this;
    access_sys_t *p_sys = p_access->p_sys;
    msg_Err(p_this, "hliu close url%s", p_sys->url);
    vlc_UrlClean( &p_sys->url );
    http_auth_Reset( &p_sys->auth );
    vlc_UrlClean( &p_sys->proxy );
    http_auth_Reset( &p_sys->proxy_auth );
...
{
	msg_Info_Br("hliu:%s","call ReadData");
    access_sys_t *p_sys = p_access->p_sys;
    if( p_sys->b_chunked )
    {
        if( p_sys->i_chunk < 0 )
...
{
	msg_Info_Br("hliu:%s","call Connect");
    access_sys_t   *p_sys = p_access->p_sys;
...
static int Request( access_t *p_access, uint64_t i_tell )
{
	msg_Info_Br("hliu:%s","call Request");
    access_sys_t   *p_sys = p_access->p_sys;
    char           *psz ;
	...
	struct CustomHeader* headers = NULL;
	int custom_header_size =  GetCustomHeaders(&headers);
	for(int i = 0; i < custom_header_size; ++i)
	{
        net_Printf(p_access, p_sys->fd, pvs, "%s: %s\r\n", headers[i].key, headers[i].value);
	}
