...
int net_Connect( vlc_object_t *p_this, const char *psz_host, int i_port,
                 int type, int proto )
{
	msg_Info_Br("hliu:%s","call net_connect host:%s, port%d", psz_host? psz_host:"None", i_port);
	...
	msg_Info_Br("hliu i_realport%d",i_realport);
    if (val)
    {
	...
	msg_Info_Br("hliu2:connect:%u port%d",addr_in->sin_addr, addr_in->sin_port);
...
static int SocksHandshakeTCP( vlc_object_t *p_obj,
                              ...
                              const char *psz_host, int i_port )
{
	//msg_Info_Br("hliu:%s","call SockHandshakeTCP");
    uint8_t buffer[128+2*256];
